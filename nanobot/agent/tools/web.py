...
class WebToolsConfig(Base):
    ...
    # Local: accept `enabled` as an alias for backward compatibility with older configs.
...
class WebSearchTool(Tool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # Local: timeout wrap so a hung provider doesn't stall the agent turn.
        # (Upstream's execute() body inlined here as the awaited coroutine.)
        self._refresh_config()
        provider = self.config.provider.strip().lower() or "brave"
