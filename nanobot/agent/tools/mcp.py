...
def _is_session_terminated(exc: BaseException) -> bool:
    ...
    Matches either:
    - any transient/stream-dead exception type (``_is_transient`` covers the
      anyio stream-closed set plus connection-reset/refused/aborted — these
      carry no error message, so route them straight to reconnect rather than
      the futile sleep-and-retry against a dead session)
    ...
    *,
    ...
    async def open_single_server(
        ...
        try:
            ...
            # Two OAuth paths coexist, selected by the type of cfg.auth:
            #   * cfg.auth == "oauth"  -> upstream's browser flow (MCP SDK httpx.Auth)
            #   * isinstance(cfg.auth, OAuthConfig) -> fork's headless flow (device_code /
            #     client_credentials): resolve a bearer token and inject the header.
            # Both re-run on every reconnect, so refresh-token rotation happens for free.
            ...
            elif cfg.auth == "oauth" and transport_type not in {"sse", "streamableHttp"}:
                logger.warning(
            ...
            if transport_type == "stdio":
                ...
                http_client_kwargs: dict[str, Any] = {
                    # Fork: request_headers (not cfg.headers) so our headless-OAuth
                    # bearer header reaches the streamableHttp transport too.
