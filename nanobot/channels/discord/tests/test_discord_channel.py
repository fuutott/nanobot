...
@pytest.mark.asyncio
async def test_on_message_ignores_other_bot_without_mention() -> None:
    # Fork: other bots are ignored unless they explicitly mention this bot.
    channel = DiscordChannel(DiscordConfig(enabled=True, allow_from=["*"]), MessageBus())
    channel._bot_user_id = "999"
    handled: list[dict] = []
    channel._handle_message = lambda **kwargs: handled.append(kwargs)  # type: ignore[method-assign]

    await channel._on_message(_make_message(author_id=123, author_bot=True, content="ping"))
