...
def test_optional_dependency_metadata_for_enable():
    ...
    # Fork: we deliberately re-add `discord` and `telegram` as extras so
    # `uv tool install "git+...[discord,telegram]"` and the Docker build install
    # those channel deps at build time (the manifest runtime installer can't run
    # in our non-root/offline container). Exempt them from upstream's
    # "no channel name is a pyproject extra" policy.
    assert (channel_names - {"discord", "telegram"}).isdisjoint(deps)
