...
class WebUIChannel(BaseChannel):
    ...
    def __init__(self, config: WebUIConfig | dict[str, object], bus: MessageBus):
        ...
        # Session tokens -> authenticated principal (empty = auth disabled)
    ...
    def is_allowed(self, sender_id: str) -> bool:
        """Allowlist check for web sender composite IDs.

        sender_id can contain pipe-separated aliases, for example:
        "webuser:alice|alice|web:192.168.1.2|192.168.1.2".
        Accept if any alias is explicitly allowlisted.
        """
    ...
    def _token_principal(self, token: str | None) -> str | None:
        if not self._auth_enabled():
            return None
        if not token:
            return None
        principal = self._tokens.get(token)
        return principal if principal else None
    ...
    def _register_routes(self, app: FastAPI) -> None:
        ...
        async def auth_status(request: Request) -> JSONResponse:
            ...
            token_valid = self._check_token(token or None) if auth_enabled else True
        ...
        async def websocket_endpoint(websocket: WebSocket, chat_id: str, token: str = "") -> None:
            ...
            principal = self._token_principal(token or None)
            sender_parts = [f"web:{client_host}", client_host]
            if principal:
                sender_parts = [f"webuser:{principal}", principal, *sender_parts]
